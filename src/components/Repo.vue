<template>
  ...
  <v-card class="mx-auto">
    ...
    <v-toolbar color="indigo" dark>
      ...
      <v-text-field flat solo-inverted hide-details prepend-inner-icon="search" label="URL"
        hint="e.g. http://enca.co.za/laksjdalk" class="hidden-sm-and-down pa-2" v-model="url"></v-text-field>
      ...
      <template v-for="(post, index) in filteredList">
        ...
        <v-list-item v-else :key="post.url" @click>
          ...
          <v-list-item-content>
            ...
            <v-list-item-subtitle>Transaction Hash: <a :href="post.thash">{{post.thash}}</a></v-list-item-subtitle>
          </v-list-item-content>
        </v-list-item>
          ...
          <v-row justify="space-between">
            <v-col cols="12">
              ...
              <v-badge v-bind:key="index" right>
                <template v-slot:badge>{{post.down_votes}}</template>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col>
                    <v-text-field hint="e.g. 0x1lk2jlkasd..." label="Your ethereum address" v-model="user_address"
...
<script>
  ...
  export default {
    data: () => ({
      ...
      url: "",
      urls: [],
      web3: new Web3(new Web3.providers.HttpProvider("http://localhost:11000")),
      amangaContract: {},
      ...
      contractAddress: "0x6fb9f7c63227735302421cd803bb6850903bcbe6",
 ...
 */
    ...
    methods: {
      ...
      vote: async function () {
        let This = this
        this.isLoading = true
        if (this.web3.utils.isAddress(this.user_address)) {
          console.log('current_post: ',this.web3.utils.toHex(this.selected_post.url))
          if (this.down_vote) {
            this.amangaContract.methods.up_down_vote_url(this.web3.utils.toHex(this.selected_post.url), 1, this
              .user_address).send({
              gas: 8000000,
            ...
            this.amangaContract.methods.up_down_vote_url(this.web3.utils.toHex(this.selected_post.url), 0, this
              .user_address).send({
              gas: 8000000,
              from: This.relayAddress
            }).then((receipt, err) => {
              ...
              console.log('err: ', err)
              This.isLoading = false
              This.error('Something went wrong!! you seem to have ran out of daily votes')
            })
          }
        } else {
        ...
        console.log('voting: ', this.selected_post)
      },
                ...
                This.amangaContract.methods
                  ...
                  .then((details, err) => {
                    This.urls.push({
                      ...
                      img: "https://image.flaticon.com/icons/svg/1862/1862164.svg",
                      title: this.web3.utils.toAscii(key),
                      up_votes: parseInt(details[1]),
                      down_votes: parseInt(details[2]),
                      author: details[0],
                      thash: 'https://ropsten.etherscan.io/tx/'+details[3],
